...
type UserQuery struct {
	...
	withOutgoingFriendRequests *FriendRequestQuery
	withIncomingFriendRequests *FriendRequestQuery
...
// QueryOutgoingFriendRequests chains the current query on the "outgoing_friend_requests" edge.
func (uq *UserQuery) QueryOutgoingFriendRequests() *FriendRequestQuery {
	query := &FriendRequestQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, user.OutgoingFriendRequestsTable, user.OutgoingFriendRequestsColumn),
		)
		fromU = sqlgraph.SetNeighbors(uq.driver.Dialect(), step)
		return fromU, nil
	}
...
// QueryIncomingFriendRequests chains the current query on the "incoming_friend_requests" edge.
func (uq *UserQuery) QueryIncomingFriendRequests() *FriendRequestQuery {
	query := &FriendRequestQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(user.Table, user.FieldID, selector),
			sqlgraph.To(friendrequest.Table, friendrequest.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, user.IncomingFriendRequestsTable, user.IncomingFriendRequestsColumn),
		)
		fromU = sqlgraph.SetNeighbors(uq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withOutgoingFriendRequests: uq.withOutgoingFriendRequests.Clone(),
		withIncomingFriendRequests: uq.withIncomingFriendRequests.Clone(),
...
// WithOutgoingFriendRequests tells the query-builder to eager-load the nodes that are connected to
// the "outgoing_friend_requests" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithOutgoingFriendRequests(opts ...func(*FriendRequestQuery)) *UserQuery {
	query := &FriendRequestQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withOutgoingFriendRequests = query
	return uq
}

// WithIncomingFriendRequests tells the query-builder to eager-load the nodes that are connected to
// the "incoming_friend_requests" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithIncomingFriendRequests(opts ...func(*FriendRequestQuery)) *UserQuery {
	query := &FriendRequestQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withIncomingFriendRequests = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	var (
		...
		loadedTypes = [3]bool{
			uq.withFriends != nil,
			uq.withOutgoingFriendRequests != nil,
			uq.withIncomingFriendRequests != nil,
		}
	)
	...
	if query := uq.withOutgoingFriendRequests; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.OutgoingFriendRequests = []*FriendRequest{}
		}
		query.Where(predicate.FriendRequest(func(s *sql.Selector) {
			s.Where(sql.InValues(user.OutgoingFriendRequestsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			node, ok := nodeids[fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "source_id" returned %v for node %v`, fk, n.ID)
			}
			node.Edges.OutgoingFriendRequests = append(node.Edges.OutgoingFriendRequests, n)
		}
	}

	if query := uq.withIncomingFriendRequests; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.IncomingFriendRequests = []*FriendRequest{}
		}
		query.Where(predicate.FriendRequest(func(s *sql.Selector) {
			s.Where(sql.InValues(user.IncomingFriendRequestsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			node, ok := nodeids[fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "destination_id" returned %v for node %v`, fk, n.ID)
			}
			node.Edges.IncomingFriendRequests = append(node.Edges.IncomingFriendRequests, n)
		}
	}
