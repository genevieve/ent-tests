...
type FriendRequestQuery struct {
	...
	// eager-loading edges.
	withSource      *UserQuery
	withDestination *UserQuery
...
func (frq *FriendRequestQuery) QuerySource() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, friendrequest.SourceTable, friendrequest.SourceColumn),
		)
...
func (frq *FriendRequestQuery) QueryDestination() *UserQuery {
	query := &UserQuery{config: frq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := frq.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(friendrequest.Table, friendrequest.FieldID, selector),
			sqlgraph.To(user.Table, user.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, friendrequest.DestinationTable, friendrequest.DestinationColumn),
		)
		fromU = sqlgraph.SetNeighbors(frq.driver.Dialect(), step)
		return fromU, nil
	}
...
// Returns a *NotSingularError when exactly one FriendRequest entity is not found.
...
// Returns a *NotSingularError when exactly one FriendRequest ID is not found.
...
func (frq *FriendRequestQuery) Clone() *FriendRequestQuery {
	...
	return &FriendRequestQuery{
		...
		withSource:      frq.withSource.Clone(),
		withDestination: frq.withDestination.Clone(),
...
func (frq *FriendRequestQuery) WithSource(opts ...func(*UserQuery)) *FriendRequestQuery {
	query := &UserQuery{config: frq.config}
	for _, opt := range opts {
		opt(query)
	}
	frq.withSource = query
	return frq
}
...
func (frq *FriendRequestQuery) WithDestination(opts ...func(*UserQuery)) *FriendRequestQuery {
	query := &UserQuery{config: frq.config}
	for _, opt := range opts {
		opt(query)
	}
	frq.withDestination = query
	return frq
}
...
//		GroupBy(friendrequest.FieldSourceID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.FriendRequest.Query().
//		Select(friendrequest.FieldSourceID).
//		Scan(ctx, &v)
//
...
func (frq *FriendRequestQuery) sqlAll(ctx context.Context) ([]*FriendRequest, error) {
	...
	if query := frq.withSource; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "source_id" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := frq.withDestination; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*FriendRequest)
		for i := range nodes {
			fk := nodes[i].DestinationID
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "destination_id" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Destination = n
			}
		}
	}
